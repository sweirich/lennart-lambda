...
--
--
module Bound.NbE (nf,Bound.Def.aeq,toDB,fromDB,nfd, impl) where
...
import Bound.Def (DB (..), fromDB, toDB, aeq)
...
-- 1. Done
-- 2. Hoasures which don't wait for a value.
-- 3. Or do.
--
data Sem b where
    Done :: DB b                                   -> Sem b
...
lower (Hoas f) = DLam $ toScope $ lower $ f F (Done (DVar (B ())))
...
eval2 env (DLam (Scope (DVar (F b)))) = Skip $
    eval2 env b
eval2 env (DLam (Scope (DVar (B x)))) = Hoas $ \_ren t ->
    t
eval2 env (DLam b) = Hoas $ \ren t ->
